...
def add_handlers(bot: TelegramClient):
    ...
    '''bot.add_event_handler(
        handle_ytdl_command,
        events.NewMessage(pattern=r"(https?://)?(www\.)?(youtube|youtu|youtube-nocookie)\.(com|be)/(watch\?v=|embed/|v/|.+\?v=)?(?P<id>[A-Za-z0-9\-=_]{11})",
        chats=get_val("ALD_USR"))
    )'''
    #magnet\:\?xt\=urn\:btih\:([A-F\d]+)
    bot.add_event_handler(
        handle_leech_command,
        events.NewMessage(pattern=r"magnet:\?xt[^\s]+",
        chats=get_val("ALD_USR"))
    )
    bot.add_event_handler(
        handle_leech_command,
    ...
    bot.add_event_handler(
        handle_leech_command,
        events.NewMessage(func=lambda e: e.document,
        chats=get_val("ALD_USR"))
    )
    ...
    '''bot.add_event_handler(
        ...
        events.NewMessage(pattern=r"https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)",
        chats=get_val("ALD_USR"))
    )'''
    ...
    '''bot.add_event_handler(
        ...
        events.NewMessage(pattern=r'^https:\/\/www\.instagram\.com\/(p|tv|reel)\/([A-Za-z0-9\-_]*)\/(\?igshid=[a-zA-Z0-9]*)?$',
        chats=get_val("ALD_USR"))
    )'''
...
async def handle_leech_command(e):
    try:
        ...
        if e.video:
            torlog.info("\n\n File Detected! Discarding.. \n\n")
            return
        if e.document:
            name = None
            for i in e.document.attributes:
                if isinstance(i,types.DocumentAttributeFilename):
                    name = i.file_name
                    torlog.info("{}".format(name))
            ...
            link = str(e.message.message)
            if ("youtube.com" in link ) or ("youtu.be" in link) or ("t.me" in link) or ("instagram" in link):
                torlog.info("\n\n Link detected! Cancelling \n\n")        
                return
            '''for line in badwordfile:
                line=line[:-1]
                if line in link:    
                    torlog.info("\n\n P*rn Link detected! Cancelling \n\n")
                    await e.delete()        
                    return'''
    except:pass
    rclone = False
