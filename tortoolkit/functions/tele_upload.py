...
from ..functions import Leech_Module
import os
import logging
import time
import traceback
...
async def upload_handel(path, message, from_uid, files_dict, job_id=0, force_edit=False, updb=None, from_in=False, thumb_path=None, user_msg=None, task=None):
    ...
    if os.path.isdir(path):
        ...
        if os.path.getsize(path) > get_val("TG_UP_LIMIT"):
            ...
            '''if "www.TamilBlasters" in path:
                dldirname = os.path.dirname(path)
                newfilename = "/"
                newfilename += os.path.basename(path).split("- ")[1]
                newpath = dldirname + newfilename
                torlog.info("newpath:")
                torlog.info(newpath)
...
async def upload_a_file(path, message, force_edit, database=None, thumb_path=None, user_msg=None):
    ...
    torlog.info("from upload a file")
    torlog.info(os.path.basename(path))
    if "www.tamilblasters" in os.path.basename(path).lower() or "www.1tamilmv" in os.path.basename(path).lower():
        dldirname = os.path.dirname(path)
        newfilename = "/"
        newfilename += os.path.basename(
            path)[os.path.basename(path).index("-")+2:]
        newpath = dldirname + newfilename
        torlog.info("newpath:")
        torlog.info(newpath)
        os.rename(path, newpath)
        path = newpath
        torlog.info("renamed path")
        torlog.info(path)
    else:
        dldirname = os.path.dirname(path)
        newfilename = "/@Caption_"
        newfilename += os.path.basename(path)
        torlog.info(newfilename)
        newpath = dldirname + newfilename
        torlog.info("newpath:")
        torlog.info(newpath)
        os.rename(path, newpath)
    ...
    caption_str += "</b>"
    # caption_str += await getimdbinfo(path,file_name)

    metadata = extractMetadata(createParser(path))
    ometa = metadata

    # imdb info
    # await getimdbinfo(path,file_name)
        ...
        if message.media and force_edit:
            ...
            if ftype == "video" and not force_docs:
                ...
                try:
                    ...
                    torlog.warning(
                        "Streamable file send failed fallback to document.")
                    out_msg = await msg.client.send_file(
                        msg.to_id,
                        file=path,
                        parse_mode="html",
...
async def upload_single_file(path, message, force_edit, database=None, thumb_image_path=None, user_msg=None):
    ...
    if "www.tamilblasters" in os.path.basename(path).lower() or "www.1tamilmv" in os.path.basename(path).lower():
        dldirname = os.path.dirname(path)
        newfilename = "/"
        startindex = os.path.basename(path).index("-")
        newfilename += os.path.basename(path)[startindex+2:]
        newpath = dldirname + newfilename
        os.rename(path, newpath)
        path = newpath
    file_name = os.path.basename(path)
    ...
    # if len(file_name) > 64:
    #     teststr = file_name.rsplit(".", 1)
    #     extensionlen = len(teststr[1]) + 1
    #     newfile_name = teststr[0][:60-extensionlen] + "." + teststr[1]
    #     newpath1 = os.path.dirname(path)+"/"+newfile_name
    #     os.rename(path, newpath1)
    #     path = newpath1
    # await getimdbinfo(path)
    ...
    '''tsavepath = os.path.join(os.getcwd(), "Downloads/defaultthumb.jpg")
    tresponse = requests.get("https://i.imgur.com/UV9wGFJ.jpg")

    file = open(tsavepath, "wb")
    file.write(tresponse.content)
    file.close()
    torlog.info(tsavepath)
    thumb_image_path = tsavepath
    torlog.info(thumb_image_path)'''
    try:
        ...
        if ftype == "video" and not force_docs:
            ...
            if force_edit and message.photo:
                ...
                paths = await copy_file_path(path)
            ...
            if force_edit and message.photo:
                ...
                if ".txt" not in os.path.basename(paths[-1]) and ".nfo" not in os.path.basename(paths[-1]) and ".exe" not in os.path.basename(paths[-1]) and ".jpeg" not in os.path.basename(paths[-1]) and ".png" not in os.path.basename(paths[-1]) and ".srt" not in os.path.basename(paths[-1]) and ".sfv" not in os.path.basename(paths[-1]) and ".sub" not in os.path.basename(paths[-1]) and ".idx" not in os.path.basename(paths[-1]) and ".jpg" not in os.path.basename(paths[-1]):
                    fmsg = "<b>"+caption_str
                    fmsg+="</b>\n\n<b>Available Now..✅ \n<i>Type in Group and Take😜</i></b>"
                    await asyncio.sleep(5)
                    await message.reply(fmsg, parse_mode="html")
                for path in paths:
                    os.remove(path)
            #if thumb is not None:
                #os.remove(thumb)
...
async def copy_file_path(path):
    ...
    for prefixcaption in prefixcaptions:
        ...
        if len(newfilename) > 64:
            teststr = newfilename.rsplit(".", 1)
            extensionlen = len(teststr[1]) + 1
            newfilename = teststr[0][:60-extensionlen] + "." + teststr[1]
        copyfilepath = os.path.dirname(path) + "/Copy" + newfilename
        if os.path.isfile(copyfilepath):
            os.remove(copyfilepath)
        shutil.copyfile(path, copyfilepath)
        paths.append(copyfilepath)
    #paths.append(path)
    return paths


async def get_all_thumbpaths():
    thumb_paths = []
    thumb_paths.append(None)
    #thumburls = ["https://i.imgur.com/UV9wGFJ.jpg","https://i.imgur.com/rqe37FO.jpeg"]
    thumbnames = ["/app/kvthumb.jpg"]
    #thumbnames = [None]
    return thumbnames
